module playground::lotto_game {
    ...
    public entry fun claim_jackpot(
        ...
        let jackpot_value = balance::value(&game.jackpot_pool);
        assert!(jackpot_value > 0, ENoWinners);

        let player = tx_context::sender(ctx);
        
        // Transfer entire jackpot to player
        let jackpot_balance = balance::withdraw_all(&mut game.jackpot_pool);
        let jackpot_coin = coin::from_balance(jackpot_balance, ctx);
        transfer::public_transfer(jackpot_coin, player);
    ...
    public entry fun claim_lucky_box(
        ...
        let lucky_box_value = balance::value(&game.lucky_box_pool);
        assert!(lucky_box_value > 0, ENoWinners);

        let player = tx_context::sender(ctx);
        
        // Transfer entire lucky box to player
        let lucky_box_balance = balance::withdraw_all(&mut game.lucky_box_pool);
        let lucky_box_coin = coin::from_balance(lucky_box_balance, ctx);
        transfer::public_transfer(lucky_box_coin, player);

        event::emit(LuckyBoxWon {
            player,
            amount: lucky_box_value,
        });
    }
